...
- **Public URL**: `https://wirewarp.example.com` (front with
  Traefik/Caddy/nginx for TLS). Container port: `8000`; compose maps
  host port `8100`.
...
- Alembic for migrations — never modify schema without a migration. Current head is `0030_security_events`.
- Pydantic v2 for all request/response schemas.
- JWT (python-jose); passwords bcrypt-hashed.
- FastAPI native WebSockets at `/ws/agent` for agents and
  `/ws/dashboard` for dashboard realtime events. No socket.io.
- Tests: pytest + httpx.
- Sensitive helpers: `app/services/network_alloc.py` (per-server
  `/24` allocator), `app/services/tunnel_server_ops.py`
  (`dispatch_wg_init`, `dispatch_wg_attach`, peer add/remove, LAN
  egress/SNAT dispatch), `app/services/vpn_ops.py` (VPN endpoint/profile
  command dispatch), `app/services/traefik_ops.py` and
  `app/services/crowdsec_ops.py` (edge desired-state helpers),
  `app/services/primary_ip.py`
  (`get_primary_ip` / `resolve_public_ip` for multi-IP).

### Go agent
- Single Go binary, minimal deps. Match the toolchain to
  `wirewarp-agent/go.mod` before release work; note that CI currently pins
  Go 1.22 while `go.mod` declares a newer version.
...
- Prefer explicit `exec.Command(name, args...)`. If an installer or edge
  integration needs a shell script, keep it fixed-input and do not pass
  user-controlled strings through a shell.
- Config stored as YAML at `/etc/wirewarp/agent.yaml`.
- Mesh WireGuard private keys are generated locally; only public keys
  leave the agent. Road-warrior profile keys are generated by the server
  and returned once in the user config.
...
- React Query for server state. Realtime invalidation comes from
  `src/lib/realtime.ts`; selected security views still use scoped polling
  for status/aggregate refresh.
...
- Current schema source of truth is `wirewarp-server/app/models/` plus
  `wirewarp-server/alembic/versions/`. The SQL sketch in
  `ARCHITECTURE.md` is historical planning context.
...
2. **Mesh private keys never leave the agent** — only public keys are sent to the control server for site-to-site tunnels. VPN profile private keys are generated server-side, returned once in the config, and not stored.
3. **Offline resilience** — agents must work if the control server is down. Apply disk config first, then try to connect.
4. **No arbitrary remote shell execution** — agents only expose whitelisted command types. No eval and no user-supplied shell snippets.
5. **Install is one command** — `curl ... | bash -s -- --mode <s|c> --url <ctrl> --token <tok>`. No interactive prompts on the target machine.
6. **Gateways are inbound-only by default** — a tunnel client with `is_gateway=true` accepts inbound DNAT'd traffic from VPSes and forwards to LAN devices. It does **not** install the old blanket `from <LANNetwork> table <wg>` rule. LAN-originated egress through a VPS is explicit per discovered/pinned LAN client via `set_lan_egress`.
